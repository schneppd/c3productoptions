...
class AttributeGroupCore extends ObjectModel
{
	...
	public static function getAttributesGroups($id_lang)
	{
		if (!Combination::isFeatureActive())
			return array();
		//c3 add c3_attribute_group_required logic to prevent user confusion if checkbox required
		return Db::getInstance()->executeS('
			SELECT DISTINCT agl.`name`, ag.*, agl.*, IF(cagr.`id_attribute_group` IS NULL, false, true) AS group_required
			FROM `'._DB_PREFIX_.'attribute_group` ag
			'.Shop::addSqlAssociation('attribute_group', 'ag').'
			LEFT JOIN `'._DB_PREFIX_.'attribute_group_lang` agl
				ON (ag.`id_attribute_group` = agl.`id_attribute_group` AND `id_lang` = '.(int)$id_lang.')
			 LEFT JOIN `'._DB_PREFIX_.'c3_attribute_group_required` cagr ON ag.`id_attribute_group` = cagr.`id_attribute_group`
			ORDER BY `name` ASC
		');
	}
