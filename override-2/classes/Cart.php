...
class Cart extends CartCore
{
	...
	public function getProducts($refresh = false, $id_product = false, $id_country = null)
	{
		...
		foreach ($result as &$row)
		{
			...
			$final_product_price = 0.0;
			$price_wt = 0.0;
			foreach($c3_product_price_infos['options'] as $key => $option){
				$final_product_price += (float)$c3_product_price_infos['basic_price'] + (float)$option;
				if($key == 0)
					$price_wt = (float)$c3_product_price_infos['basic_price'] + (float)$option;
			}
			if(count($c3_product_price_infos['options']) == 0){
				$final_product_price += (float)$c3_product_price_infos['basic_price'] * $row['quantity'];
				$price_wt = (float)$c3_product_price_infos['basic_price'];
			}
			$raw_final_product_price = $final_product_price;
			$raw_price_wt = $price_wt;
			$row['price'] = $price_wt;
			...
			switch (Configuration::get('PS_ROUND_TYPE'))
			{
				...
				case Order::ROUND_LINE:
					$row['total'] = Tools::ps_round($final_product_price, _PS_PRICE_COMPUTE_PRECISION_);
					$row['total_wt'] = Tools::ps_round($tax_calculator->addTaxes($final_product_price), _PS_PRICE_COMPUTE_PRECISION_);
					break;
			...
			$row['price_wt'] = $tax_calculator->addTaxes($price_wt);
			...
			//debug $row['attributes'] = '$raw_final_product_price: '.$raw_final_product_price.' $raw_price_wt:'.$raw_price_wt.' $final_product_price: '.$final_product_price.' $price_wt:'.$price_wt.' $row[total]: '.$row['total'].' $row[total_wt]: '.$row['total_wt'];
	...
	*/
	public function getOrderTotal($with_taxes = true, $type = Cart::BOTH, $products = null, $id_carrier = null, $use_cache = true)
	{
		...
		foreach ($products as $product) // products refer to the cart details
		{
			...
			if(count($c3_product_price_infos['options']) == 0){
				$final_product_price += (float)$c3_product_price_infos['basic_price'] * (int)$product['cart_quantity'];
				$price_wt = (float)$c3_product_price_infos['basic_price'];
			}
			//$product['unit_price_tax_incl'] = $price_wt;
			$product['price_without_specific_price'] = 1;
			$price = $price_wt;
			...
			{
				case Order::ROUND_TOTAL:
					$products_total[$id_tax_rules_group.'_'.$id_address] += $final_product_price;

					if ($ecotax)
						$ecotax_total += $ecotax * (int)$product['cart_quantity'];
					break;
				case Order::ROUND_LINE:
					$product_price = $final_product_price; //$price * $product['cart_quantity'];
				...
				default:
					...
					$product_price = $with_taxes ? $tax_calculator->addTaxes($final_product_price) : $final_product_price;
					$products_total[$id_tax_rules_group] += Tools::ps_round($final_product_price, _PS_PRICE_COMPUTE_PRECISION_);
	 ...
	 */
	public function c3DeleteProduct($id_product, $id_product_attribute = null, $id_customization = null, $id_address_delivery = 0, $txtCustomInputs = "")
	{
		...
		//c3 delete all customization for given product in cart
		if(!is_null($id_product_attribute))
		{
			$sql = 'DELETE FROM '._DB_PREFIX_.'c3_cart_product_customization WHERE id_cart='.(int)$this->id.' AND id_product_attribute='.(int)$id_product_attribute.' AND id_shop='.(int)$this->id_shop;
			$result_remove = Db::getInstance()->execute($sql);
		}
