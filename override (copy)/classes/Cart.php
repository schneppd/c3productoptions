...
class CartCore extends ObjectModel
{
	...
	public static function cacheSomeAttributesLists($ipa_list, $id_lang)
	{
		...
		if(count($txtValueAttributs))
		{
			$attributes_old_value = self::$_attributesLists[$id_product_attribute.'-'.$id_lang]['attributes'];
			...
			$attributes_part = '';
			foreach ($txtValueAttributs as $id_product_attribute=>$value)
			{
				//get all txt values
				$txtValues = Db::getInstance()->executeS('
				SELECT `date_add`, `txtvalue`, `attribute_name`, `public_group_name`
				...
				foreach ($txtValues as $row)
				{
					...
					{
						$attributes_new_value .= $attributes_part;
						$attributes_new_value .= ' | ';
			...
			//store new value
			self::$_attributesLists[$id_product_attribute.'-'.$id_lang]['attributes'] = $attributes_new_value;
		}
	...
	{
		$res = updateQty($quantity, $id_product, $id_product_attribute, $id_customization, $operator, $id_address_delivery, $shop, $auto_add_cart_rule);
		...
		if($res < 0)
		{
			...
			if($operator = 'up')
			{
				...
				for($i = 0; $i < $nbAdd; $i++)
				{
					...
					foreach($txtCustomInputs as $key => $value)
					{
						$idGroupAttribute = explode('-', $key);
						$id_attribute_group = (int)$idGroupAttribute[0];
						$id_attribute = (int)$idGroupAttribute[1];
						$result_add = Db::getInstance()->insert('c3_cart_product_customization', array(
							'date_add' => $tAdd
							,'id_cart' => (int)$this->id
							,'id_product' => (int)$id_product
							,'id_product_attribute' => (int)$id_product_attribute
			...
			elseif($operator = 'down')
			{
				...
				for($i = 0; $i < $nbAdd; $i++)
				{
					$where = 'id_cart='.(int)$this->id.' AND id_product='.(int)$id_product.' AND id_product_attribute='.(int)$id_product_attribute.' AND id_shop='.$shop->id.' AND date_add = (SELECT MAX(cpc.date_add) FROM '._DB_PREFIX_.'c3_cart_product_customization AS cpc WHERE cpc.id_cart='.(int)$this->id.' AND cpc.id_product='.(int)$id_product.' AND cpc.id_product_attribute='.(int)$id_product_attribute.' AND cpc.id_shop='.$shop->id.')';
					$result = Db::getInstance()->delete('c3_cart_product_customization', $where);
					if (!$result_remove)
						return false;
				}
			}
