...
class CartControllerCore extends FrontController
{
	...
	public function init()
	{
		...
		//c3 process custom text input logic
		$txtvalues = Tools::getValue('txtvalues',NULL);
		if(!is_null($txtvalues))
		{
			$inputs = explode('-txtval-', $txtvalues);
			$txtCustomInputs = [];
			foreach($inputs as $key)
			{
				...
				//test against hack
				//if valid inputs
				if((int)$idGroupAttribute[0] > 0 && (int)$idGroupAttribute[1] > 0)
				{
					$postName = 'txtvalue-'.$key;
					$val = Tools::getValue($postName);
					$txtCustomInputs[$key] = $val;
				}
				//someone is trying to snoop around, report
				else
				{
					//todo
				}
	...
	protected function processChangeProductInCart()
	{
		...
		if (!$this->errors && $mode == 'add')
		{
			...
			if (!$this->errors)
			{
				...
				$update_quantity = $this->context->cart->updateC3Qty($this->qty, $this->id_product, $this->id_product_attribute, $this->customization_id, Tools::getValue('op', 'up'), $this->id_address_delivery, $txtCustomInputs);
